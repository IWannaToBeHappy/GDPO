...
class SpmiEval(SwiftInfer):
    ...
    def _batch_infer(self, val_dataset, request_config):
        ...
        spmis,pmis = self._cal_spmi(val_dataset, labels_list)
        if not (args.infer_backend == 'vllm' and rank >= 0 and args.rank % args.vllm_tensor_parallel_size != 0):
            for data, spmi,pmi, labels in zip(val_dataset, spmis,pmis, labels_list):
                data = {'spmi': spmi,'pmi':pmi, 'labels':labels, **data}
    ...
    def _cal_spmi(self, val_dataset, labels:List[str]) -> Dict:
        ...
        with torch.no_grad():
            ...
            prior_messages = [[{'role':'assistant', 'content':label}] for label in labels]
            prior_inputs = [StdTemplateInputs.from_dict({'messages':prior_message}) for prior_message in prior_messages]
            prior_inputs = [TemplateInputs(chosen=prior_input) for prior_input in prior_inputs]
            prior_encodeds = [template.encode_with_kwargs(prior_input, **tokenizer_kwargs) for prior_input in prior_inputs]
            prior_batch_encoded_inputs = to_device(template.data_collator(prior_encodeds), device)
            prior_outputs = model(**prior_batch_encoded_inputs)
            prior_logits = prior_outputs.logits
            prior_valid_masks = self._mask_match(prior_encodeds, labels,prior_logits)
            prior_labels = [torch.tensor(prior_encoded['labels'],device=prior_logits.device) for prior_encoded in prior_encodeds]
            prior_labels = pad_sequence(prior_labels, batch_first=True, padding_value=-100).contiguous()
            shift_prior_logits = prior_logits[:, :-1, :].contiguous()
            shift_prior_labels = prior_labels[:,1:].contiguous()
            shift_prior_masks = prior_valid_masks[:,:-1].contiguous()
            prior_token_probs = celoss(
                shift_prior_logits.view(-1, shift_prior_logits.size(-1)),
                shift_prior_labels.view(-1)
            ).view_as(shift_prior_labels)
            prior_token_probs = prior_token_probs * shift_prior_masks
            prior_token_probs = torch.exp(-prior_token_probs)  # 转为概率
            # 将有效token移至最左端，辅助先验后验向量对齐
            prior_mask_start = shift_prior_masks.int().argmax(dim=1, keepdim=True)  # 每行第一个True的位置
            prior_mask_len = shift_prior_masks.sum(dim=1)
            prior_token_align = torch.zeros_like(prior_token_probs)
            for i in range(prior_token_probs.size(0)):
                prior_token_align[i,:prior_mask_len[i]] = prior_token_probs[i,prior_mask_start[i,0]:prior_mask_start[i,0]+prior_mask_len[i]]
            ...
            # 对齐 根据tokenizer的不同，对齐后两向量可能有一位偏差，只取最小长度
            valid_len = torch.minimum(valid_masks.sum(dim=1),prior_valid_masks.sum(dim=1))
            # 对齐 张量形状对齐
            if token_align.size(1) > prior_token_align.size(1):
                token_align = token_align[:,:prior_token_align.size(1)]
            elif token_align.size(1) < prior_token_align.size(1):
                prior_token_align = prior_token_align[:,:token_align.size(1)]
            
            # cal spmi
            spmi = token_align / (token_align + prior_token_align)
            spmi_list = []
            ...
            pmi = token_align
            pmi_list = []
            for i in range(pmi.size(0)):
                if valid_len[i]>0:
                    pmi_list.append(pmi[i,:valid_len[i]].nanmean().item())
                else:
                    pmi_list.append(0.0)
        return spmi_list,pmi_list
